...
function masterEntitySheet_(entity) {
  const key = String(entity || '').trim().toUpperCase();
  const sheetName = MASTER_SHEETS[key];
  if (!sheetName) throw new Error('UNKNOWN_ENTITY');
  return sheetName;
}
function serializableValue_(value) {
...
function getMasterRows(entity) {
  ...
  let rows = readObjects_(sheetName).map(r => sanitizeClientRow_(sheetName, r));
...
function saveMasterRow(entity, data) {
  ...
  if (!data.id) {
    ...
    if (sheetName === MASTER_SHEETS.USERS) {
      ...
      if (password.length < 5) throw new Error('PASSWORD_MIN_5');
      data.password_hash = hashPassword_(password);
    ...
    if (sheetName === MASTER_SHEETS.USERS && Object.prototype.hasOwnProperty.call(data, 'password')) {
      const password = String(data.password || '');
      if (password && password.length < 5) throw new Error('PASSWORD_MIN_5');
      if (password) data.password_hash = hashPassword_(password);
    }
    delete data.password;
  ...
  if (sheetName === MASTER_SHEETS.KELAS) {
    if (!String(data.sheet_name || '').trim()) data.sheet_name = makeClassSheetName_(data.kode, data.nama);
    data.sheet_name = normalizeSheetName_(data.sheet_name);
    const actualName = createOrEnsureClassSheet_(data);
    data.sheet_name = actualName;
    updateById_(sheetName, data.id, {sheet_name: actualName, updated_at: new Date()});
  }
...
function deleteMasterRow(entity, id) {
  const user = requireOwner_();
  const sheetName = masterEntitySheet_(entity);
  if (!id) throw new Error('INVALID_DELETE_REQUEST');
...
function appendObject_(sheet, obj) {
  if (!sheet) throw new Error('SHEET_NOT_FOUND');
  const headers = (sheet.getRange(1,1,1,Math.max(sheet.getLastColumn(),1)).getValues()[0] || []).map(parseColumnId_);
  if (!headers.length) throw new Error('HEADER_NOT_FOUND');
