...
const MASTER_SHEETS = Object.freeze({
...
const MASTER_SCHEMA = Object.freeze({
  MASTER_CONFIG: [
    ['id','ID','TEXT'], ['key','Kunci','TEXT'], ['value','Nilai','TEXT'], ['updated_at','Diperbarui','DATETIME']
  ],
  MASTER_USERS: [
    ['id','ID','TEXT'], ['username','Username','TEXT'], ['password_hash','Password Hash','TEXT'],
    ['nama','Nama','TEXT'], ['role','Role','TEXT'], ['kelas_id','Kelas ID','TEXT'],
    ['guru_id','Guru ID','TEXT'], ['karyawan_id','Karyawan ID','TEXT'], ['siswa_id','Siswa ID','TEXT'],
    ['status','Status','TEXT'], ['last_login','Login Terakhir','DATETIME'], ['created_at','Dibuat','DATETIME'], ['updated_at','Diubah','DATETIME']
  ],
  ...
  MASTER_SISWA: [['id','ID','TEXT'], ['nisn','NISN','TEXT'], ['nama','Nama','TEXT'], ['kelas_id','Kelas ID','TEXT'], ['status','Status','TEXT'], ['created_at','Dibuat','DATETIME'], ['updated_at','Diubah','DATETIME']],
  MASTER_KELAS: [['id','ID','TEXT'], ['kode','Kode','TEXT'], ['nama','Nama Kelas','TEXT'], ['sheet_name','Nama Sheet','TEXT'], ['admin_user_id','Admin User ID','TEXT'], ['status','Status','TEXT'], ['created_at','Dibuat','DATETIME'], ['updated_at','Diubah','DATETIME']],
  ...
  MASTER_MENU: [['id','ID','TEXT'], ['key','Key','TEXT'], ['label','Label','TEXT'], ['icon','Icon','TEXT'], ['handler','Handler','TEXT'], ['urutan','Urutan','NUMBER'], ['roles','Roles','TEXT'], ['active','Aktif','BOOLEAN'], ['config_json','Config JSON','TEXT'], ['created_at','Dibuat','DATETIME'], ['updated_at','Diubah','DATETIME']],
...
function getOrCreateMasterSpreadsheet_() {
  ...
  props.setProperty('MASTER_OWNER_EMAIL', Session.getEffectiveUser().getEmail());
  return ss;
}
